...
const NftDetails = (props) => {
    ...
    const sendLike = async () => {
        ...
        //   setLike(like + 1);
        // }
        await axios.post(`http://localhost:5000/nfts`, { id: card_id, account: account }).then((res) => {
    ...
    async function gettokenuri(tokenId) {
        const tokenURI = await CreateNFTContract.methods.tokenURI(tokenId).call((error) => {
            if (!error) {
                console.log("send ok");
            } else {
                console.log(error);
    ...
    function testfunc(Loading) {
        if (Loading) {
            return (
                <div>
                    <ReactLoaing type={"balls"} color={"purple"} height={667} width={375} />
                </div>
            );
        } else {
            return (
