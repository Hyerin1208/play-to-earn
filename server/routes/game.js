...
router.post("/snake", async (req, res, next) => {
    ...
    try {
        ...
        if (!findAddress) {
            ...
            Game.update({ snakePoint: point, nick: findUser.nick }, { where: { address: account } });
        }
...
router.post("/2048", async (req, res, next) => {
    ...
    try {
        ...
        if (!findAddress) {
            ...
            Game.update({ puzzlePoint: score, nick: findUser.nick }, { where: { address: account } });
        }
...
router.post("/mine", async (req, res, next) => {
    ...
    try {
        ...
        if (!findAddress) {
            ...
            Game.update({ minePoint: runtime, nick: findUser.nick }, { where: { address: account } });
        }
...
router.post("/tetris", async (req, res, next) => {
    ...
    try {
        ...
        if (!findAddress) {
            ...
            Game.update({ tetrisPoint: data, nick: findUser.nick }, { where: { address: account } });
        }
...
router.post("/weekly", async (req, res) => {
    const findUser = await User.findOne({
        where: { address: "0xC7E1F2dca144AEDA8ADF4f9093da9aAC18ce7436" },
        attributes: ["weeks"],
    });
